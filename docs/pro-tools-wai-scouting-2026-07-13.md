...
|---|---|---|
| Translate a whole document | m2m100-1.2b ($0.342/M tokens each way) + local PDF text extraction already in the stack. 40-page doc ≈ $0.025 COGS; 3 credits sell for $0.055 (monthly pack) to $0.068 (starter) → 54–63% worst-case margin by pack, 80%+ typical — floor holds on every pack. Top traffic (PH/TH) is multilingual — direct fit. | **BUILT** (`translate-document-pro`, 3 credits, 40-page cap, 37-language UI incl. Filipino/Thai/Vietnamese) |
