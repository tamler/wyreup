...
- 97981c3: Fix `WYREUP_CORE_VERSION` reporting `0.0.0`.
...
- 3d038e0: **Breaking (`@wyreup/core`): `@huggingface/transformers` is now an optional peer dependency instead of a direct dependency.**
  ...
  npm `overrides` only apply in the _root_ project, so ours protect this repo but
...
- cb33668: Rename the hosted OCR tool's display name to "High-Accuracy OCR" — it shared
  the literal name "OCR" with the free on-device tool, making the pair
  indistinguishable on tool pages, suggestions, and agent tool lists. Ids and
  slugs are unchanged.
...
- bf1651d: Eight new tools from the phase-2 opportunities review, batch 2: merge-audio and audio-speed (the thinnest category, riding the existing ffmpeg engine — join clips, change speed with natural pitch), images-to-gif (animated GIF from stills via gifenc), epub-to-text (completes the document trio next to pdf-to-text/docx-to-text), har-sanitize (strip cookies/tokens/auth from network captures before sharing — redacts in place, stays DevTools-loadable), vcf-to-csv (contacts to spreadsheet, sibling of ics-to-csv), toml-convert (TOML⇄JSON via smol-toml), and ip-subnet-calc (CIDR math with correct /31 and /32 handling). All free, all local, all with curated chain suggestions that lead with free tools.

## 0.7.0

### Minor Changes

- 9fe2e7a: Six new tools from the phase-2 opportunities review: heic-to-jpg (iPhone HEIC/HEIF to JPG/PNG/WebP via libheif wasm — unlocks Apple photos for every image tool), compress-image-to-size (hit an exact KB target for passport/visa/upload forms; binary-searches quality then optionally downscales; never touches already-small files), image-collage (strip or grid from 2-20 photos), pixelate-region (block-pixelate a region — or everything — before sharing screenshots), ics-to-csv (calendar exports to CSV/JSON for the spreadsheet suite), and x509-decode (PEM/DER certificate to a readable report with SANs, validity, and fingerprints). All run entirely in the browser, all wired into chain suggestions.
...
- 42a4e90: Truth-in-metadata pass from the phase-1 tool review: descriptions now disclose actual behavior (csv-json array requirement, csv-merge dropped key column, color-palette transparency handling, word-counter markdown counting, pdf-flatten no-op passthrough, image-diff JSON stats output, pdf-metadata read-mode JSON, url-build spec-param input, strip-video-metadata container-only scope); bg-remove and color-blind-simulator declare their real image/\* output; roman-numeral no longer claims file input it ignores.
...
- 955165a: Compress never returns a larger file than its input: when a same-format re-encode comes out bigger (flat-color PNGs, screenshots), the original bytes are returned untouched. Also renames the hosted upscale tool to "Upscale (Hosted GPU)" to distinguish it from the free in-browser "Upscale 2x".
...
- b556b2e: Security dependency bumps: js-yaml to ^4.3.0 (quadratic-complexity DoS in merge-key handling, affects >= 4.0.0 <= 4.1.1) and diff to ^8.0.3 (DoS in parsePatch/applyPatch in the 6.x-7.x line).
