...
describe('password-generator — run()', () => {
  ...
  it('count > 1 generates multiple lines', async () => {
    const [out] = (await passwordGenerator.run([], { length: 16, count: 5 }, makeCtx())) as Blob[];
  ...
  it('each generated password is different (statistically)', async () => {
    const [out] = (await passwordGenerator.run([], { length: 16, count: 10 }, makeCtx())) as Blob[];
  ...
  it('throws when count > 1000', async () => {
    await expect(passwordGenerator.run([], { count: 1001 }, makeCtx())).rejects.toThrow(
      'count must be <= 1000',
    );
  });

  it('throws when count < 1', async () => {
    await expect(passwordGenerator.run([], { count: 0 }, makeCtx())).rejects.toThrow(
      'count must be >= 1',
    );
  });
});
