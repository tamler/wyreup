...
import {
  ...
  shouldInclude,
} from '../../../src/tools/zip-extract/index.js';
...
describe('zip-extract — shouldInclude()', () => {
  it('includes all when filter is undefined', () =>
    expect(shouldInclude('file.txt', undefined)).toBe(true));
  ...
  it('does not match different extension', () =>
    expect(shouldInclude('hello.png', '*.txt')).toBe(false));
});
...
describe('zip-extract — run()', () => {
  ...
  it('throws when no entries match filter', async () => {
    ...
    await expect(zipExtract.run([zip], { filter: '*.mp4' }, makeCtx())).rejects.toThrow(
      /no files found/i,
    );
  });
});
