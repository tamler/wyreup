...
let dir: string;
beforeEach(async () => {
  dir = await mkdtemp(join(tmpdir(), 'wymcp-audit-'));
});
afterEach(async () => {
...
describe('Auditor [spec §#6]', () => {
  ...
  it('loose mode swallows write errors (logger called, no throw)', async () => {
    ...
    const a = new Auditor({
      path: '/nonexistent-dir-xyz/x.jsonl',
      strict: false,
      apiKey: undefined,
  ...
  it('strict mode throws on write failure', async () => {
    ...
    await expect(
      a.append({ ts: 'x', tool: 't', input_paths: [], status: 'ok', duration_ms: 0 }),
    ).rejects.toThrow();
  });
});
